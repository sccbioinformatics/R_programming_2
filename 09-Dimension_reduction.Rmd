...
Clustering is a very common step where cells with similar gene expression profiles are grouped together. There are a choice of algorithms, and in R Programming 1 we implemented hierarchical and kmeans clustering. In single-cell analysis, cells are clustered by generating a graph on which an algorithm is applied to find communities (clusters). We are going to apply Louvine clustering on the data here using.

This bit is trickier, so we'll do it together.
...
dd.pr <- prcomp(t(req.data),center=FALSE)$x
```
...
```{r,eval=F}
library(RANN)
snn <- nn2(dd.pr[,1:10], k=30)$nn.idx # calculate the graph using 30 max nearest neighbours using he first 10 principal components

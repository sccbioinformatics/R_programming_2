...
A recap on accing data objects:
...
You can see that it complains because it expected a martrix, but we tried to give it a data frame. This is why S4 classes as useful, there is a sanity check before the object is created so all downstream process work as expected. Speaking of sanity, what do we think of this?
...
***Exercise:*** Create a function called `CreateMySCO` which take a data matrix (`pbmc.data`) and creates an mysco class containing the raw unfiltered data and the `features`/`nCounts` in the meta slot. Call the resulting object `pbmc`, hence over-writing the current one.
